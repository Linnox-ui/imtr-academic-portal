...
export default async function LecturerTimetablePage() {
  ...
  })) satisfies LecturerTimetableEntry[];

  const lecturerName =
    [currentUser.firstName, currentUser.lastName].filter(Boolean).join(" ") ||
    currentUser.email;
  const uniqueIntakes = new Set(typedEntries.map((entry) => entry.intake.code));
  const uniqueUnits = new Set(
    typedEntries.map((entry) => entry.unitAssignment.unit.code),
  );
    ...
    <div className="space-y-6">
        ...
        <div className="relative flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="animate-in fade-in slide-in-from-left-3 duration-500">
            ...
            <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-slate-400">
              View approved class sessions assigned to {lecturerName}.
            </p>
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <a
              href={`/lecturer/timetable/download`}
              target="_blank"
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          ...
          helper="Weekly timetable entries"
        />
        <StatCard
          icon={Clock3}
          label="Hours"
          value={totalHours}
          helper="Teaching hours per week"
        />
        <StatCard
          icon={BookOpen}
          label="Units"
          value={uniqueUnits.size}
          helper="Assigned teaching units"
        />
        <StatCard
          icon={Users}
          label="Classes"
          value={uniqueIntakes.size}
          helper="Linked intakes"
        />
      </section>
          ...
          {typedEntries.length ? (
            <section className="hidden xl:block overflow-hidden rounded-[26px] border border-slate-200 bg-slate-100/80 shadow-sm">
              <SectionHeader
                ...
                subtitle="Read-only schedule generated from coordinator records."
              />
          ...
          {entriesByDay.length ? (
            <section className="xl:hidden overflow-hidden rounded-[26px] border border-slate-200 bg-slate-100/80 shadow-sm">
              <SectionHeader
                ...
                subtitle="Your teaching sessions broken down by day."
              />
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs font-semibold text-slate-500">
                  This page is lecturer-specific.
                </p>
              </div>
            </div>
            <div className="mt-5 space-y-3">
              <GuideItem text="Only sessions assigned to you are shown." />
...
function TimetableRow({
  ...
  entries: LecturerTimetableEntry[];
}) {
  return (
    <>
...
function TimetableEntryCard({ entry }: { entry: LecturerTimetableEntry }) {
  return (
    <div className="h-full rounded-xl border border-sky-200 bg-sky-50 p-3">
      ...
      <p className="mt-2 text-[11px] font-bold leading-4 text-slate-600">
        {entry.intake.code} · {entry.unitAssignment.semester.courseYear.title} ·{" "}
...
function SessionRow({ entry }: { entry: LecturerTimetableEntry }) {
  return (
    <div className="flex flex-col gap-3 py-4 first:pt-5 last:pb-5 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        ...
        <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">
          {entry.intake.code} · {entry.intake.course.code} ·{" "}
          {entry.unitAssignment.semester.courseYear.title} ·{" "}
          {entry.unitAssignment.semester.title}
        </p>
      </div>
      <div className="flex flex-wrap gap-2 sm:justify-end">
        <SmallBadge icon={Users} text={entry.intake.code} />
        {entry.room ? <SmallBadge icon={MapPin} text={entry.room} /> : null}
      </div>
    </div>
  );
...
function MobileSessionCard({ entry }: { entry: LecturerTimetableEntry }) {
  return (
  ...
  return (
    <div className="m-5 flex min-h-72 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-200/40 px-5 text-center sm:m-6">
      ...
      <p className="mt-4 text-lg font-black text-slate-800">
        No timetable sessions yet
      </p>
      <p className="mt-2 max-w-md text-sm font-semibold leading-6 text-slate-500">
        Your timetable will appear after the course coordinator schedules
        approved units assigned to you.
      </p>
    </div>
  );
}

...
export async function GET() {
  try {
    ...
    let csvContent = "\uFEFF"; // UTF-8 BOM so Excel reads it properly
    csvContent +=
    ...
    // 5. Return the downloadable file and force the browser to NOT cache it
    return new NextResponse(csvContent, {
      status: 200,
      headers: {
        "Content-Type": "text/csv; charset=utf-8",
        "Content-Disposition":
          'attachment; filename="IMTR_Academic_Performance_Report.csv"',
    ...
    return new NextResponse("Internal Server Error generating CSV", {
      status: 500,
    });
  }
}

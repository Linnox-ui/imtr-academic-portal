...
export async function GET(req: NextRequest) {
  try {
    ...
    if (intakeId) {
      targetIntakes = await prisma.intake.findMany({
        where: { id: intakeId },
        include: { course: true },
      });
    } else {
      const activeEntries = await prisma.timetableEntry.findMany({
        where: { isActive: true },
        select: { intakeId: true },
      });

      const activeIntakeIds = [
        ...new Set(activeEntries.map((e) => e.intakeId)),
      ];

      targetIntakes = await prisma.intake.findMany({
        where: { id: { in: activeIntakeIds } },
        include: { course: true },
    ...
    if (targetIntakes.length === 0) {
      return new Response("No timetable data found", { status: 404 });
    }

    const intakeIds = targetIntakes.map((i) => i.id);
    ...
    const pdfBuffer = await new Promise<Buffer>((resolve, reject) => {
      try {
        ...
        targetIntakes.forEach((intake, index) => {
          ...
          doc
            ...
            .font("Helvetica")
            .fillColor("#64748b")
            .text("OFFICIAL CLASS TIMETABLE", { align: "center" });
          doc.moveDown(1.5);

          let currentY = doc.y;

          doc.roundedRect(40, currentY, 515, 50, 8).fill("#f8fafc");
          doc
            .roundedRect(40, currentY, 515, 50, 8)
            .lineWidth(1)
            .stroke("#e2e8f0");
          ...
          doc
            .fontSize(10)
            .font("Helvetica-Bold")
            .text("Class Code:", 55, currentY + 18);
          doc.font("Helvetica").text(intake.code, 125, currentY + 18);

          doc.font("Helvetica-Bold").text("Course:", 55, currentY + 34);
          doc
            .font("Helvetica")
            .text(
              `${intake.course.code} - ${intake.course.title}`,
              125,
              currentY + 34,
            );
          ...
          const intakeEntries = allEntries.filter(
            (e) => e.intakeId === intake.id,
          );
          ...
          const calculateCellHeight = (classes: any[]) => {
            ...
            classes.forEach((c) => {
              ...
              h += 12;
              h +=
                doc.heightOfString(title, { width: colWidths.block - 20 }) + 4;
              h += 12;
              h += 8;
            });
            return h + 10;
          ...
          const drawCellContent = (classes: any[], x: number, y: number) => {
            ...
            classes.forEach((c) => {
              ...
              cy +=
                doc.heightOfString(titleStr, { width: colWidths.block - 20 }) +
                4;

              const roomText = c.room ? ` • Rm: ${c.room}` : "";
              const lectStr = `${c.lecturer.firstName} ${c.lecturer.lastName}${roomText}`;
              doc
                .fillColor("#64748b")
                .font("Helvetica")
                .fontSize(8)
                .text(lectStr, x + 10, cy, { width: colWidths.block - 20 });
              cy += 16;
            });
          };

          DAYS.forEach((day) => {
            const dayEntries = intakeEntries.filter(
              (e) => String(e.dayOfWeek) === day,
            );
            ...
            doc
              ...
              .stroke("#e2e8f0");
            doc
              .rect(cols.mid, currentY, colWidths.block, maxH)
              .stroke("#e2e8f0");
            doc
        ...
        for (let i = 0; i < pages; i++) {
          ...
          doc
            ...
            .fillColor("#94a3b8")
            .text(
              `Generated by Academic Directorate on ${new Date().toLocaleString("en-GB")}.`,
              40,
              780,
              { align: "center" },
            );
    ...
    const filename = intakeId
      ? `timetable-${targetIntakes[0].code}.pdf`
      : `master-timetables-imtr.pdf`;

...
import { createIntake } from "@/app/actions/intake.actions";
...
type NewIntakeFormProps = {
  courses: CourseOption[];
  lecturers: LecturerOption[];
};

export function NewIntakeForm({ courses, lecturers }: NewIntakeFormProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      ...
      if (result?.success) {
        toast.success(result.message || "Intake created.");

        router.push("/academic-director/intakes");
      ...
      <section className="relative overflow-hidden rounded-[28px] bg-slate-900 px-5 py-6 text-white shadow-xl shadow-slate-900/10 sm:px-7">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-start gap-4">
            ...
            <div className="min-w-0 animate-in fade-in slide-in-from-left-3 duration-500">
              ...
              <p className="mt-2 text-sm font-semibold text-slate-400">
                Create a cohort for admissions.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 sm:flex">
            ...
            <HeroStat label="Year" value={currentYear} />
          </div>
        </div>
      </section>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">
              ...
              <div className="space-y-2 sm:col-span-2">
                ...
                <input
                  ...
                  id="title"
                  disabled={isPending}
                  placeholder="Meteorological Middle Training Course 2026 Intake"
          ...
          <section className="overflow-hidden rounded-[26px] border border-slate-200 bg-slate-100/80 shadow-sm">
            <SectionHeader
              icon={BookOpen}
              title="Linked course"
              subtitle="Select the parent course."
            />
              ...
              <div className="space-y-2">
                ...
                {courses.length === 0 ? (
                  <p className="text-xs font-bold text-amber-700">
                    Create a course before adding an intake.
                  </p>
                ) : null}
              </div>
              ...
              {selectedCourse ? (
                <div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-200/50 p-4 sm:grid-cols-2">
                  ...
                  <PreviewItem
                    icon={BadgeCheck}
                    label="Category"
                    value={formatEnum(selectedCourse.category)}
                  />
          ...
          <section className="overflow-hidden rounded-[26px] border border-slate-200 bg-slate-100/80 shadow-sm">
            <SectionHeader
              ...
              subtitle="Assign one lecturer to coordinate this intake."
            />
            ...
            <div className="space-y-3 p-5 sm:p-6">
              <div className="space-y-2">
                ...
                <select
                  ...
                  name="coordinatorId"
                  id="coordinatorId"
                  disabled={isPending || lecturers.length === 0}
                  className="h-12 w-full rounded-2xl border border-slate-300 bg-slate-200/60 px-4 text-sm font-black text-slate-950 outline-none transition-all focus:border-sky-600 focus:bg-slate-100 focus:ring-4 focus:ring-sky-600/10 disabled:opacity-60"
                >
                ...
                {lecturers.length === 0 ? (
                  <p className="text-xs font-bold text-amber-700">
                    Create an active lecturer account before assigning a
                    coordinator.
                  </p>
                ) : null}
              </div>
              ...
              <div className="rounded-2xl border border-sky-200 bg-sky-50 p-4">
                <p className="text-xs font-bold leading-5 text-sky-800">
                  The selected lecturer will coordinate this intake only. The
                  account remains a lecturer account.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="overflow-hidden rounded-[26px] border border-slate-200 bg-slate-100/80 shadow-sm">
            <SectionHeader
              ...
              subtitle="Assessment mode and current status."
            />
            ...
            <div className="grid gap-4 p-5 sm:grid-cols-2 sm:p-6">
              <SelectField
                ...
                defaultValue="CAT_AND_FINAL_EXAM"
                disabled={isPending}
                options={[
          ...
          <FormFooter
            href="/academic-director/intakes"
            isPending={isPending}
            disabled={courses.length === 0 || lecturers.length === 0}
            label="Save Intake"
          />
        </form>
    ...
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        <input
          ...
          className={`h-12 w-full rounded-2xl border border-slate-300 bg-slate-200/60 pl-11 pr-4 text-sm font-black text-slate-950 outline-none transition-all placeholder:font-semibold placeholder:text-slate-400 focus:border-sky-600 focus:bg-slate-100 focus:ring-4 focus:ring-sky-600/10 ${
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-xs font-semibold text-slate-500">
              Keep cohort records clean.
            </p>
          </div>
        </div>

        <div className="mt-5 space-y-3">
          <GuideItem text="Use a unique intake code." />
          <GuideItem text="Year supports admissions and reports." />
          <GuideItem text="Assign one lecturer as the intake coordinator." />
          <GuideItem text="Assessment mode controls results handling." />
        </div>
      </div>
    </aside>
  );

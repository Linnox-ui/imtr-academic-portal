...
import { updateCourseUnit } from "@/app/actions/course-unit.actions";
...
type EditCourseUnitFormProps = {
  unit: UnitRecord;
  courses: CourseOption[];
};

export function EditCourseUnitForm({
  unit,
  courses,
}: EditCourseUnitFormProps) {
  const [
    selectedCourseId,
    setSelectedCourseId,
  ] = useState(unit.courseId);
  ...
  const handleSubmit = (
    ...
    startTransition(async () => {
      const result =
        await updateCourseUnit(formData);
      ...
      router.push(
        `/academic-director/course-units/${unit.id}`,
      );
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-start gap-4">
            <Link
              ...
              aria-label="Back to course unit"
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={isPending}
                    autoComplete="off"
                    className="h-12 w-full rounded-2xl border border-slate-300 bg-slate-200/60 pl-11 pr-4 font-mono text-sm font-black uppercase tracking-wide text-slate-950 outline-none transition-all focus:border-sky-600 focus:bg-slate-100 focus:ring-4 focus:ring-sky-600/10 disabled:opacity-60"
                  />
                </div>
              </div>
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-slate-200 bg-slate-200/50 p-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">
                    Inactive units stay saved but are not used for new records.
                  </p>
                </div>
              </label>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="mt-0.5 text-xs font-semibold text-slate-500">
                  Keep academic records stable.
                </p>
              </div>
            </div>

            <div className="mt-5 space-y-3">
              <GuideItem text="Avoid changing codes after results exist." />
              <GuideItem text="Move units only after official approval." />
              <GuideItem text="Deactivate old units instead of deleting them." />
            </div>
          </div>
        </aside>
      </div>

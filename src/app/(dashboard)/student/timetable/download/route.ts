...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!intakeId) {
      return new Response("Missing intakeId", { status: 400 });
    }
    ...
    const pdfBuffer = await new Promise<Buffer>((resolve, reject) => {
      try {
        ...
        doc
          .fontSize(10)
          .font("Helvetica-Bold")
          .text("Class Code:", 55, currentY + 15);
        doc.font("Helvetica").text(intake.code, 125, currentY + 15);

        doc.font("Helvetica-Bold").text("Course:", 55, currentY + 35);
        doc
          .font("Helvetica")
          .text(
        ...
        const admission = currentUser.studentProfile?.admissionNumber || "N/A";

        doc.font("Helvetica-Bold").text("Student:", 300, currentY + 15);
        doc.font("Helvetica").text(studentName, 355, currentY + 15);
        doc.font("Helvetica-Bold").text("Admission:", 300, currentY + 35);
        doc.font("Helvetica").text(admission, 355, currentY + 35);
    ...
    return new Response(pdfBuffer as unknown as BodyInit, {
      ...
      headers: {
        ...
        "Content-Disposition": `attachment; filename="timetable-${intake.code}.pdf"`,
      },
    });
  } catch (error) {
